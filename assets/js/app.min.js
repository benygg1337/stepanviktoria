...
window.addEventListener('load', () => {
  if (document.querySelector('.in10-wrapper')) {
    setTimeout(() => {
      document.querySelector('.in10-wrapper').classList.add('_loaded');
    }, 0);
  }
});
...
function body_lock_add(delay) {
  ...
  if (unlock) {
    ...
    for (let index = 0; index < lock_padding.length; index++) {
      const el = lock_padding[index];
      el.style.paddingRight = `${window.innerWidth - document.querySelector('.in10-wrapper').offsetWidth}px`;
    }
    body.style.paddingRight = `${window.innerWidth - document.querySelector('.in10-wrapper').offsetWidth}px`;
...
if (moreBlocks.length > 0) {
  const wrapper = document.querySelector('.in10-wrapper');
...
if (in4Timer.length > 0) {
  ...
  const date = new Date('2025/04/26 00:00:00');
  ...
  function counts() {
    ...
    if (elementD) elementD.innerText = `${days}`;
    if (elementH) elementH.innerText = (`0${hours}`).slice(-2);
    if (elementM) elementM.innerText = (`0${minutes}`).slice(-2);
    if (elementS) elementS.innerText = (`0${seconds}`).slice(-2);
    ...
    document.querySelectorAll('.timer__d').forEach((el) => {
      el.textContent = declOfNum(days, ['день', 'дня', 'дней']);
    });

    document.querySelectorAll('.timer__h').forEach((el) => {
      el.textContent = declOfNum(hours, ['час', 'часа', 'часов']);
    });

    document.querySelectorAll('.timer__m').forEach((el) => {
      el.textContent = declOfNum(minutes, ['минута', 'минуты', 'минут']);
    });

    document.querySelectorAll('.timer__s').forEach((el) => {
      el.textContent = declOfNum(seconds, ['секунда', 'секунды', 'секунд']);
    });
  }
...
document.addEventListener('DOMContentLoaded', () => {
  const popup = document.querySelector('.popup_thanks');
  ...
  async function initializeRecaptcha() {
    return new Promise((resolve, reject) => {
      grecaptcha.enterprise.ready(() => {
        grecaptcha.enterprise.execute('6LePpo8qAAAAAFgVL8KPGbSejjXDeMHI13ISu6dh', { action: 'submit' }).then((token) => {
